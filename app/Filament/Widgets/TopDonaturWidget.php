...
class TopDonaturWidget extends BaseWidget
{
    ...
    protected function calculateStatistics(): void
    {
        $stats = Donatur::query()
            ...
            ->whereNotIn('donasis.jenis_donasi_id', JenisDonasi::where('nama', 'Penyaluran Langsung')->select('id'))
            ->select([
            ...
            ->filters([
                ...
                SelectFilter::make('jenis_donasi')
                    ...
                    ->query(function (Builder $query, array $data): Builder {
                        ...
                        return $query->when(
                            ...
                            fn (Builder $query, $jenisId): Builder => $query->where('donasis.jenis_donasi_id', $jenisId)
                        );
                    }),
                ...
                SelectFilter::make('provinsi')
                    ...
                    ->query(function (Builder $query, array $data): Builder {
                        return $query->when(
                            $data['value'],
                            fn (Builder $query, $provinsiId): Builder => $query->where('donaturs.province_id', $provinsiId)
                        );
                    }),
                ...
                Filter::make('minimal_donasi')
                    ...
                    ->query(function (Builder $query, array $data): Builder {
                        return $query->when(
                            $data['jumlah'],
                            fn (Builder $query, $jumlah): Builder => $query->havingRaw('SUM(COALESCE(donasis.jumlah, 0) + COALESCE(donasis.perkiraan_nilai_barang, 0)) >= ?', [$jumlah])
                        );
                    })
                ...
                Filter::make('minimal_frekuensi')
                    ...
                    ->query(function (Builder $query, array $data): Builder {
                        return $query->when(
                            $data['frekuensi'],
                            fn (Builder $query, $frekuensi): Builder => $query->havingRaw('COUNT(donasis.id) >= ?', [$frekuensi])
                        );
                    })
                    ->indicateUsing(function (array $data): ?string {
